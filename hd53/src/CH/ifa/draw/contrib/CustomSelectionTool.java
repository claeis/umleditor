	...
	protected void showPopupMenu(Figure figure, int x, int y, Component comp) {
		...
		if ((attribute != null) && (attribute instanceof JPopupMenu)) {
			...
			// calculate offsets for internal MDI frames
				//START BUG FIX => if zoomed in a ZoomDrawingView
                double scale=1.0;
                DrawingView v=view();
                if (v instanceof CH.ifa.draw.contrib.zoom.ZoomDrawingView) {
                    scale=((CH.ifa.draw.contrib.zoom.ZoomDrawingView)v).getScale();
                }
				Point newLocation = new Point((int)(x*scale),(int)(y*scale));
				adjustOffsets(comp /*.getParent() -> BugFix phirzel*/, newLocation);
			    //END BUG FIX

...
/**
 * Delegate mouse selection to a specific TextTool if the figure selected inside
 * a CompositeFigure is a TextFigure.
 *
 * @author: Peter Hirzel <i>soft</i>Environment
>>>>>>> 803fe805af2eebe1581931014fa25d7f5559e1e9
...
=======
	...
	// java.util.ResourceBundle.getBundle("ch/ehi/umleditor/umldrawingtools/resources/DelegationSelectionTool");
	// //$NON-NLS-1$
>>>>>>> 803fe805af2eebe1581931014fa25d7f5559e1e9
...
=======
	...
	public DelegationSelectionTool(DrawingEditor editor) {
		...
		chxShowRole = new javax.swing.JCheckBoxMenuItem(
				NodeFigure.getResourceString(DelegationSelectionTool.class, "ChxShowRole_text")); //$NON-NLS-1$
		chxShowRole.setActionCommand(ROLES_ACTION_COMMAND);
		chxShowRole.addActionListener(this);
		chxShowRole.setSelected(((ClassDiagramView) view()).isShowAllRoles());

		chxShowMultiplicity = new javax.swing.JCheckBoxMenuItem(
				NodeFigure.getResourceString(DelegationSelectionTool.class, "ChxShowMultiplicity_text")); //$NON-NLS-1$
		chxShowMultiplicity.setActionCommand(CARDINALITY_ACTION_COMMAND);
		chxShowMultiplicity.addActionListener(this);
		chxShowMultiplicity.setSelected(((ClassDiagramView) view()).isShowMultiplicities());

		chxShowAssociationNames = new javax.swing.JCheckBoxMenuItem(
				NodeFigure.getResourceString(DelegationSelectionTool.class, "ChxShowAssociationNames_text"));
		chxShowAssociationNames.setActionCommand(ASSOCIATION_NAME_ACTION_COMMAND);
		chxShowAssociationNames.addActionListener(this);
		chxShowAssociationNames.setSelected(((ClassDiagramView) view()).isShowAssociationNames());

		chxShowAttributes = new javax.swing.JCheckBoxMenuItem(
				NodeFigure.getResourceString(DelegationSelectionTool.class, "ChxShowAttributes_text"));
		chxShowAttributes.setActionCommand(ATTRIBUTES_ACTION_COMMAND);
		chxShowAttributes.addActionListener(this);
		chxShowAttributes.setSelected(((ClassDiagramView) view()).isShowAttributes());

		chxShowAttributeTypes = new javax.swing.JCheckBoxMenuItem(
				NodeFigure.getResourceString(DelegationSelectionTool.class, "ChxShowAttributeTypes_text"));
		chxShowAttributeTypes.setActionCommand(ATTRIBUTE_TYPES_ACTION_COMMAND);
		chxShowAttributeTypes.addActionListener(this);
		chxShowAttributeTypes.setSelected(((ClassDiagramView) view()).isShowAttributeTypes());

		chxShowAttributeCardinality = new javax.swing.JCheckBoxMenuItem(
				NodeFigure.getResourceString(DelegationSelectionTool.class, "ChxShowAttributeCardinality_text"));
		chxShowAttributeCardinality.setActionCommand(ATTRIBUTE_CARDINALITY_ACTION_COMMAND);
		chxShowAttributeCardinality.addActionListener(this);
		chxShowAttributeCardinality.setSelected(((ClassDiagramView) view()).isShowAttributeMultiplicity());

		chxShowLinkFigure = new javax.swing.JCheckBoxMenuItem(
				NodeFigure.getResourceString(DelegationSelectionTool.class, "ChxShowLinkFigure_text"));
		chxShowLinkFigure.setActionCommand(LINK_FIGURE_ACTION_COMMAND);
		chxShowLinkFigure.addActionListener(this);
		chxShowLinkFigure.setSelected(((ClassDiagramView) view()).isShowLinkFigure());

		chxShowOperations = new javax.swing.JCheckBoxMenuItem(
				NodeFigure.getResourceString(DelegationSelectionTool.class, "ChxShowOperations_text"));
		chxShowOperations.setActionCommand(OPERATION_ACTION_COMMAND);
		chxShowOperations.addActionListener(this);
		chxShowOperations.setSelected(true); // ((ClassDiagramView)view()).isShowLinkFigure());
		...
		mnuFormat.add(new AbstractAction(ch.ehi.basics.i18n.ResourceBundle.getBundle(DelegationSelectionTool.class)
				.getString("MenuLayoutRolesCardinalities")) { //$NON-NLS-1$
			public void actionPerformed(ActionEvent event) {
				mniLayoutRolesAndMultiplicities();
			}
		});
	...
	private int checkDragCase(ConnectionFigure edgeFigure, int x, int y) {
		...
		if (wayPointIndex > 0) {
			((EdgeFigure) edgeFigure).getClassDiagram().removeXorNote(((EdgeFigure) edgeFigure).getModelElement());
			// TODO trigger PresentationRoleFigure#updateFigure() to draw
			// XOR-Note
			return MOVE_WAYPOINT;
		} else if ((edgeFigure instanceof PresentationRoleFigure)
				|| (edgeFigure instanceof GeneralizationLineConnection)
			...
			if (iterator.hasNext()) {
				...
				if (((dragSourceNode instanceof ch.ehi.umleditor.umlpresentation.Package)
						|| (dragSourceNode instanceof Note)) && isWithinRange(edgeFigure.startPoint(), x, y)) {
			...
			if (iterator.hasNext()) {
				...
				if (((dragSourceNode instanceof ch.ehi.umleditor.umlpresentation.Class)
						|| (dragSourceNode instanceof ch.ehi.umleditor.umlpresentation.Package)
						|| (dragSourceNode instanceof Note)) && isWithinRange(edgeFigure.endPoint(), x, y)) {

					// end Point of connection
					endPointIndex = 1; // wayPoints are kept in a separate
										// ArrayList
					return MOVE_ENDPOINT;
				}
			} else {
	...
	protected Tool createDragTracker(Figure f) {
		if (f instanceof ModelElementUI) {
			ch.ehi.umleditor.application.LauncherView.getInstance()
					.setDescription(((ModelElementUI) f).getModelElement());
		}
...
=======
	...
	private void dragAssociation(Connector end, Figure targetFigure, int x, int y) {
		// 1) Presentation
		// ((EdgeFigure)editedConnection).getEdge().setEndpoint(endPointIndex,
		// ((NodeFigure)targetFigure).getNode());
		// 2) Model
		((ch.ehi.interlis.associations.RoleDef) ((EdgeFigure) editedConnection).getModelElement()).changeParticipant(
				(ch.ehi.uml1_4.foundation.core.Classifier) ((NodeFigure) targetFigure).getModelElement());
		// 3) show visual reconnect
		/*
		 * if (endPointIndex == 0) {
		 * ((EdgeFigure)editedConnection).setStartConnector(end); } else { //
		 * (endPointIndex == 1)
		 * ((EdgeFigure)editedConnection).setEndConnector(end); }
		 */
	}
	...
	private void dragDependency(Connector end, Figure targetFigure, int x, int y) {
		...
		ch.ehi.uml1_4.foundation.core.Dependency dependency = (ch.ehi.uml1_4.foundation.core.Dependency) ((EdgeFigure) editedConnection)
				.getModelElement();
		if (endPointIndex == 0) {
			// 2) Model
			dependency.changeClient((ModelElement) dependency.iteratorClient().next(),
					(ch.ehi.uml1_4.foundation.core.GeneralizableElement) ((NodeFigure) targetFigure).getModelElement());
			// 3) show visual reconnect
			// ((EdgeFigure)editedConnection).setStartConnector(end);
		} else /* (endPointIndex == 1) */ {
			// 2) Model
	...
	private void dragGeneralization(Connector end, Figure targetFigure, int x, int y) {
		...
		ch.ehi.uml1_4.foundation.core.Generalization generalization = (ch.ehi.uml1_4.foundation.core.Generalization) ((EdgeFigure) editedConnection)
				.getModelElement();
		if (endPointIndex == 0) {
			// 2) Model
			generalization.changeChild(
					(ch.ehi.uml1_4.foundation.core.GeneralizableElement) ((NodeFigure) targetFigure).getModelElement());
			// 3) show visual reconnect
			// ((EdgeFigure)editedConnection).setStartConnector(end);
		} else /* (endPointIndex == 1) */ {
			// 2) Model
			generalization.changeParent(
					(ch.ehi.uml1_4.foundation.core.GeneralizableElement) ((NodeFigure) targetFigure).getModelElement());
			// 3) show visual reconnect
			// ((EdgeFigure)editedConnection).setEndConnector(end);
		}
	...
	/**
	 * Disconnect current node and connect new target. This kind of line is not
	 * duplicated on other Diagrams, therefore a visual relocation is provoqued
	 * here instead of doing it within a ClassDiagrammView.
	 */
	private void dragNoteAnchor(Connector end, Figure targetFigure, int x, int y) {
		if (endPointIndex == 0) {
			if (targetFigure instanceof NoteFigure) {
				...
				ch.softenvironment.view.BaseDialog.showWarning(
						(java.awt.Component) ch.ehi.umleditor.application.LauncherView.getInstance(),
						NodeFigure.getResourceString(DelegationSelectionTool.class, "CTModellingError"),
						NodeFigure.getResourceString(DelegationSelectionTool.class, "CENoteToNoteError"));
			}
		} else {
			// 1) Presentation
			((EdgeFigure) editedConnection).getEdge().setEndpoint(endPointIndex, ((NodeFigure) targetFigure).getNode());
	...
	/**
	 * Return the text tool to which double clicks are delegated. The text tool
	 * is shared by all figures upon which this selection tool operates.
	 *
	 * @return delegate text tool
	 */
	private JHotDrawTextTool getTextTool() {
	...
	/**
	 * Overwrites. Allow renaming of composite TextFigure-Contents.
	 */
	protected void handleMouseDoubleClick(MouseEvent e, int x, int y) {
		int splitX = e.getX();
		int splitY = e.getY();
		...
		if (editedConnection != null) {
			wayPointIndex = ((EdgeFigure) editedConnection).getWayPointIndex(x, y);
			if (wayPointIndex > 0) {
				// make sure same WAY_POINT_DIAMETER is checked for dragging and
				// deleting
				// editedConnection.joinSegments(splitX, splitY);
				((EdgeFigure) editedConnection).removePointAt(wayPointIndex);
			}
	...
	protected void handleMouseDown(MouseEvent e, int x, int y) {
		try {
			...
			if (view() instanceof ClassDiagramView) {
				...
				// LinkFigure linkFigure = ((ClassDiagramView)view()).fin
				if ((editedConnection != null) // &&
				// (editedConnection.getStartConnector() != null) &&
				// (editedConnection.getEndConnector() != null)) /* experienced
				// while dragging edges near a LinkFigure */ {
				) {
					// only spliting is possible, new Connections must be done
					// by appropriate tools
					dragCase = checkDragCase(editedConnection, splitX, splitY);
					// ch.softenvironment.util.Tracer.getInstance().debug("DragCase
					// = " + dragCase);
				}
			...
			ch.softenvironment.view.BaseDialog.showError(ch.ehi.umleditor.application.LauncherView.getInstance(), null,
					exception.getMessage(), exception);// $NON-NLS-1$
		}
	}

	/**
	 * Overwrites. Show either Popup for Figure at given position otherwise for
	 * current Diagram.
	 */
	protected void handlePopupMenu(MouseEvent e, int x, int y) {
		Figure figure = drawing().findFigure(e.getX(), e.getY());
		if (figure != null) {
			...
			menu.add(
					new AbstractAction(NodeFigure.getResourceString(DelegationSelectionTool.class, "MenuSaveDiagram")) { //$NON-NLS-1$
						public void actionPerformed(ActionEvent event) {
							mniSaveDiagram();
						}
					});
	 ...
	 */
	private boolean isWithinRange(Point p, int x, int y) {
		int sensitiveSquareLength = ch.ehi.umleditor.application.LauncherView.getSettings().getConnectorZone()
				.intValue();
		return ((java.lang.Math.abs(p.x - x) < sensitiveSquareLength) && (Math.abs(p.y - y) < sensitiveSquareLength));
	}

	/**
	 * Layout the Figures in Diagram automatically. ClassDiagramView
	 * popupMenu-Function.
	 */
	private void mniFindModel() {
		// TODO
	...
	/**
	 * Print the Diagram. ClassDiagramView popupMenu-Function.
	 */
	private void mniPrintDiagram() {
		ch.ehi.umleditor.application.LauncherView.getInstance().printClassDiagram((ClassDiagramView) view());
	}
	...
	public void mouseDrag(MouseEvent e, int x, int y) {
		try {
			if (!e.isPopupTrigger()) {
				if (editedConnection == null) {
					...
					case MOVE_ENDPOINT: {
						...
						Figure targetFigure = ((ClassDiagramView) view()).findConnectableFigure(x, y, editedConnection);
						if (targetFigure != null) {
							Connector newEnd = ((ClassDiagramView) view())
									.findNodeConnector(((NodeFigure) targetFigure).getModelElement(), x, y);
							...
							if (((NodeFigure) targetFigure).getNode().getClass().equals(dragSourceNode.getClass())) {
								...
								ch.softenvironment.view.BaseDialog.showWarning(
										(java.awt.Component) ch.ehi.umleditor.application.LauncherView.getInstance(),
										NodeFigure.getResourceString(DelegationSelectionTool.class, "CTModellingError"),
										NodeFigure.getResourceString(DelegationSelectionTool.class,
												"CEDragNodeIncompatibility"));
							}
			...
			ch.softenvironment.view.BaseDialog.showError(ch.ehi.umleditor.application.LauncherView.getInstance(),
					NodeFigure.getResourceString(DelegationSelectionTool.class, "CTModellingError"), //$NON-NLS-1$

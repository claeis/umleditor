...
/**
 * Figure Specification for all Elements treated as edges in an
 * UML-ClassDiagram.
 * 
 * @see NodeFigure
 *
...
=======
	...
	protected void addFormatMenu(javax.swing.JPopupMenu popupMenu) {
		...
		/*
		 * JPopupMenu fontSizeMenu = new JPopupMenu();
		 * fontSizeMenu.setName("Schriftgr̦sse"); formatMenu.add(fontSizeMenu);
		 */
		formatMenu.add(new AbstractAction(CommonUserAccess.getMniFormatFontText()) {
			public void actionPerformed(ActionEvent event) {
				mniFont();
		...
		/*
		 * formatMenu.add(new
		 * AbstractAction(CommonUserAccess.getMniFormatFillColorText()) { public
		 * void actionPerformed(ActionEvent event) { mniFillColor(); } });
		 */
	...
	/**
	 * Save the presentation in Model which will cause Diagram to draw this
	 * edge. Interpretation of WayPoint-Iterator: - first => start of
	 * LineConnection - second => end of LineConnection
	 */
	protected void addModelElement(ModelElement relationship) {
		setModelElement(relationship);
		getEdge().addSubject(relationship);
	...
	protected final void connectNodes() {
		try {
			...
			startPoint(x, y);
			Connector start = null;
			if ((getModelElement() instanceof ch.ehi.uml1_4.foundation.core.Generalization)
					&& (getStartElement() instanceof ch.ehi.uml1_4.foundation.core.Association)) {
				start = getClassDiagram().findAssociationAttributeConnector(getStartElement(), x, y, getModelElement());
			} else {
				start = getClassDiagram().findNodeConnector(getStartElement(), x, y);
			}
			if (start == null) {
				Tracer.getInstance().developerWarning(
						"AUTO-CORRECT: Missing StartNode->there must have been an improper deletion of nodes/edges before=>" //$NON-NLS-1$
								+ getSourceName(getStartElement()));// $NON-NLS-2$
				// shouldWarn(NlsUtils.formatMessage(resEdgeFigure.getString("CWMissingStartNode"),
				// getSourceName(getStartElement()))); //$NON-NLS-1$
				removeVisually();
			} else if (getEdge().sizeEndpoint() == 2) {
				// end -> assume dummy value
				x = 0;
				...
				Object second = iterator.next();
				Connector end = null;
				if ((second instanceof PresentationAssocClass) && ((getModelElement() instanceof RoleDef)
						|| ((getModelElement() instanceof ch.ehi.uml1_4.foundation.core.Generalization)))) {
					// LinkFigure -> must be an AssociatenDef connected to
					// a AssociationAttributeFigure
					// @see ElementFactory#createPresentationRole(..)
					end = getClassDiagram().findAssociationAttributeConnector(getEndElement(), x, y, getModelElement());
				...
				if (end == null) {
					Tracer.getInstance().developerWarning(
							"AUTO-CORRECT: Missing EndNode->there must have been an improper deletion of nodes/edges before=>" //$NON-NLS-1$
									+ getSourceName(getEndElement()));// $NON-NLS-2$
					// shouldWarn(NlsUtils.formatMessage(resEdgeFigure.getString("CWMissingEndNode"),
					// getSourceName(getEndElement()))); //$NON-NLS-1$
					removeVisually();
				} else {
					connectStart(start);
					setEndConnector(end);
	...
	protected int getWayPointIndex(int x, int y) {
		for (int i = 1; i < (fPoints.size() - 1); i++) {
			Point p = pointAt(i);
			if (CH.ifa.draw.util.Geom.length(x, y, p.x, p.y) < ch.ehi.umleditor.application.LauncherView.getSettings()
					.getConnectorZone().intValue()) {
				// removePointAt(i);
				return i;
			}
		}
	...
	/**
	 * Overwrites. Keep way Point in PresentationModel as well.
	 * 
	 * @see ConnectionTool.mouseDown(MouseEvent e, int x, int y)
	 *      #splitSegment(int x, int y)
	 */
	 ...
	 */
	/*
	 * private void mniFillColor() { // makes no sense here mniLineColor(); }
	 */
	/**
	 * Font Action.
	 * 
	...
	/**
	 * Overwrites. Remove WayPoint from Model.
	 * 
	 * @see #joinSegments(int x, int y)
	 */
	public void removePointAt(int i) {
	...
	/**
	 * Remove the Figure visually ONLY in its Diagram. Still kept in real model.
	 */
	public void removeVisually() {
		try {
			// 1) remove drawing
			if (getClassDiagram() != null) {
				...
				if ((edge != null) && getClassDiagram().getDiagram().containsPresentationElement(edge)) {
					...
					getClassDiagram().getDiagram().removePresentationElement(edge);
					/*
					 * // remove WayPoints edge.clearWayPoint();
					 * 
					 * if (edge.containsSubject(getModelElement())) { // remove
					 * from model edge.removeSubject(getModelElement()); }
					 */
				}
			}
		} catch (Throwable e) {
	...
	/**
	 * Extend Attribute-map with Popup-Menu and make sure LineDecorations change
	 * Colors as well.
	 */
	public void setAttribute(String name, Object value) {
		if (name.equals(JHotDrawConstants.FRAME_COLOR)) {
	 ...
	 * Set the ClassDiagram drawing this Edge.
	 *
	 * Attention: the ClassDiagram should be assigned as last action for new
	 * established edges, so updateView() may be sure to do the right thing.
	 *
	 * @see #updateView()
	 * @see NodeFigure
	 */
	...
	/**
	 * Set the Edge and its Coordinates. WayPoints are kept in an own ArrayList
	 * therefore endPoint-List contains only two entries: [0] => start [1] =>
	 * end
	 * 
	 * @see #connectNodes()
	 */
	...
	/**
	 * Overwrites. Set/reset a WayPoint with the given index at location p.
	 * Save/update the WayPoint in PresentationModel as well.
	 * 
	 * @see ConnectionTool.mouseDrag(MouseEvent e, int x, int y)
	 */
	...
	/**
	 * Check wether user must be warned because of illegal Relationship. Several
	 * mouse events might trigger canConnect(Figure, Figure) but only one
	 * Warning is desired.
	 * 
	 * @see canConnect(Figure, Figure)
	 */
	...
	/**
	 * Decorate the RelationShip-Ends. Overwrite this method for specific
	 * behaviour.
	 */
	protected void showDecoration() {
		setStartDecoration(null);
	...
	protected void updateImmediately() {
		/*
		 * comment by Wolfram changing a figure and drawing the changes (->
		 * updating) are two separate methods. This provides more flexibility if
		 * you have a sequence of changes but want to redraw only once. Thus,
		 * changed() marks the figure as dirty (e.g calling figureInvalidated())
		 * whereas figureRequestUpdate() performs the redraw afterwards
		 * (unfortunatelly the programmer must take care of this).
		 * 
		 * I admit that in most circumstances figureChanged() and
		 * figureRequestUpdate() should probably do the same. The concept is
		 * inconsistent in so far that one time you call a method (changed) on a
		 * figure to do something and the other time you notify a listener
		 * (figureRequestUpdate). Perhaps we need a method (Figure.update() ?)
		 * that makes this more consistent.
		 */
		willChange();
		if (listener() != null) {
			listener().figureRequestUpdate(new FigureChangeEvent(this));

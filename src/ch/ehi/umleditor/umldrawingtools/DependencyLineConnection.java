...
=======
 * Draw a dependency line between two Figures representing dependable
 * ModelElement's. A dependency relation is a uses-a relation with a direction
 * where the connection points to the class used by another one. The start class
 * (Client) is dependend on the end class (Supplier). A DependencyLineConnection
 * has an arrow at the end point and is dotted.
 *
 * @author Peter Hirzel <i>soft</i>Environment
 * @version $Revision: 1.7 $ $Date: 2006-06-29 22:16:01 $
 */
public class DependencyLineConnection extends EdgeFigure {
	 ...
	 * Used to display a given Model-Dependency in given ClassDiagram.
	 */
	public DependencyLineConnection(ClassDiagramView classDiagram, Figure start, Figure end,
			ch.ehi.uml1_4.foundation.core.Dependency dependency) {
		super(classDiagram);
		setEdge(new ch.ehi.umleditor.umlpresentation.Dependency(), start, end);
		addModelElement((ModelElement) dependency);
	}
	...
	/**
	 * Tests whether the two figures may be connected. Overwrites.
	 *
	 * @param start
	 *            figure representing the start/client class which is dependend
	 *            on the end class
	 * @param end
	 *            figure representing the end/supplier class
	 */
	public boolean canConnect(Figure start, Figure end) {
		Object /* GeneralizableElement */ client = ((NodeFigure) start).getModelElement(); // getGeneralizableElement(start);
		Object /* GeneralizableElement */ supplier = ((NodeFigure) end).getModelElement(); // getGeneralizableElement(end);

		if (!((((client instanceof Classifier) || ClassFigure.isPseudoClassifier(client))
				&& ((supplier instanceof Classifier) || ClassFigure.isPseudoClassifier(supplier))) ||
		// ((client instanceof TopicDef) && (supplier instanceof TopicDef)) ||
				((client instanceof ch.ehi.uml1_4.modelmanagement.Package)
						&& (supplier instanceof ch.ehi.uml1_4.modelmanagement.Package)))) {
			shouldWarn(resDependencyLineConnection.getString("CWNodesMustBeClassifier")); //$NON-NLS-1$
			return false;
		}
	...
	/**
	 * Draw the line which is a dotted line for a dependency connection. Instead
	 * of drawing one line from start point to end point, the line is divided
	 * into several small lines each 5 pixels long and 5 pixels away from the
	 * previous line. Some minor inaccuracy are possible due to rounding errors
	 * or incomplete last lines.
	 *
	 * @param g
	...
	/**
	 * Return the starting/client by means dependent ModelElement of the
	 * Relationship.
	 * 
	 * @return Element
	 */
	protected Element getStartElement() {
		if (getModelElement() == null) {
			...
			if (it.hasNext()) {
				...
				Tracer.getInstance()
						.developerError("Dependency should have had at least a client => modelling problem?");
				return null;
			}
		}
	}
	...
	protected void handleConnect(Figure start, Figure end) {
		try {
			...
			BaseDialog.showError(LauncherView.getInstance(), CREATION_ERROR,
					resDependencyLineConnection.getString("CEDependencyNotEstablished"), e); //$NON-NLS-1$
		}
	}
	...
	public void updateView() {
		if ((getModelElement() != null)
				&& (!((ch.ehi.uml1_4.foundation.core.Dependency) getModelElement()).iteratorSupplier().hasNext())) {
			removeVisually();
		} else {
			super.updateView();
		}

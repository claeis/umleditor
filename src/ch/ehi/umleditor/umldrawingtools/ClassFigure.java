...
/**
 * A ClassFigure is a graphical representation for an UML Class in an UML Class
 * Diagram. A ClassFigure separates the graphical representation from the data
 * model (PresentationElement). A class has a class name, attributes and
 * methods.
 *
...
public class ClassFigure extends NodeFigure implements ActionListener {
	...
	// Direct reference to a composite figure which stores text figures for
	// attribute names.
	...
	public void actionPerformed(java.awt.event.ActionEvent e) {
		try {
			if (e.getActionCommand().equals(SUPPRESS_ATTRIBUTES_ACTION_COMMAND)) {
				...
				((PresentationAbstractClass) getNode())
						.setShowInheritedAttributes(chxShowInheritedAttributes.isSelected());
			}
	...
	protected void addSpecialMenu(javax.swing.JPopupMenu popupMenu) {
		if (getModelElement() instanceof AbstractClassDef) {
			...
			chxSuppressAttributes = new javax.swing.JCheckBoxMenuItem(
					resources.getString("MniSuppressAttributes_text")); //$NON-NLS-1$
			chxSuppressAttributes.setActionCommand(SUPPRESS_ATTRIBUTES_ACTION_COMMAND);
			chxSuppressAttributes.addActionListener(this);
			chxShowInheritedAttributes = new javax.swing.JCheckBoxMenuItem(
					resources.getString("MniShowInheritedAttributes_text")); //$NON-NLS-1$
			...
			chxSuppressOperations = new javax.swing.JCheckBoxMenuItem(
					resources.getString("MniSuppressOperations_text")); //$NON-NLS-1$
			...
			if (getNode() == null) {
				...
				chxShowInheritedAttributes
						.setSelected(((PresentationAbstractClass) getNode()).isShowInheritedAttributes());
	...
	/**
	 * ClassFigure movement is different from AssociationAttributeFigure
	 * movement. Overwrites.
	 */
	protected void basicMoveBy(int dx, int dy) {
		...
		while (edges.hasNext()) {
			ch.ehi.umleditor.umlpresentation.PresentationEdge edge = (ch.ehi.umleditor.umlpresentation.PresentationEdge) edges
					.next();
			Figure figure = getClassDiagram().findFigure(edge);
			if ((figure != null) && (figure instanceof PresentationRoleFigure)
	 ...
	 * causes calculating the layout of contained figures.
	 */
	private void updateAttributeFigure() {
		// TODO Tune: currently all attributeFigure's are removed and readded at
		// modelChange");//$NON-NLS-2$//$NON-NLS-1$
		...
		if (((PresentationAbstractClass) getNode()).isSuppressAttributes()) {
			...
			if (((PresentationAbstractClass) getNode()).isShowInheritedAttributes()) {
				...
				java.util.List inheritedAttributes = ((ch.ehi.interlis.modeltopicclass.AbstractClassDef) getModelElement())
	...
	/**
	 * Update the method figure and the ClassFigure itself as well. This causes
	 * calculating the layout of contained figures.
	 */
	private void updateOperationFigure() {
		// TODO Tune: currently all operationFigure's are removed and readded at
		// modelChange
		operationsFigure.removeAll();
		operationSeparator.setLineVisible(false); // automatic display acc. to
													// existing methods

		if ((getModelElement() instanceof Classifier)
				&& (!((PresentationAbstractClass) getNode()).isSuppressOperations())) {
	...
	private void createEnumerationFigure(ch.ehi.interlis.domainsandconstants.basetypes.Enumeration type,
			String offset) {
	...
	protected static boolean isPseudoClassifier(Object element) {
		...
		return (element instanceof RoleDef) || (element instanceof MetaDataUseDef) || (element instanceof UnitDef)
				|| (element instanceof FunctionDef) || (element instanceof LineFormTypeDef) ||
				// (element instanceof DomainDef) ||
				(element instanceof GraphicParameterDef) || (element instanceof ViewDef)
				|| (element instanceof GraphicDef) || (element instanceof TopicDef);
	}
}

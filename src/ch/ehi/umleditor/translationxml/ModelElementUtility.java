...
public class ModelElementUtility {
	 ...
	 */
	private static ModelElement findElementInEnumeration(String scopedNamePrefix, String baselanguage, Enumeration enumeration,
			String scopedNameToFind) {
		...
		while (enumEle.hasNext()) {
			...
			ModelElement ele = findIliModelElement_Helper(enumElement, scopedNamePrefix, baselanguage, scopedNameToFind);
			if (ele != null) {
				return ele;
			}
			if (enumElement.containsChild()) {
				String scopedName = scopedNamePrefix + "." + enumElement.getName().getValue(baselanguage);
				ModelElement modelElement = findElementInEnumeration(scopedName, baselanguage, enumElement.getChild(), scopedNameToFind);
	 ...
	 * @param scopedNameToFind Expected Scope Name
	...
	private static ModelElement findIliModelElement_Helper(ModelElement modelElement, String scopedNamePrefix,
		...
		if (scopedNamePrefix.equals(scopedNameToFind)) {
			return modelElement;
		}
		...
		if (modelElement instanceof AbstractClassDef) {
			...
			while (childi.hasNext()) {
				...
				if (object instanceof AttributeDef) {
					ModelElement modelEle = findIliModelElement_Helper((ModelElement) object, scopedNamePrefix, baseLanguage, scopedNameToFind);
					if (modelEle != null) {
						return modelEle;
					}
				}
			...
			if (modelElement instanceof AssociationDef) {
				...
				while (assoDefI.hasNext()) {
					...
					ModelElement modelEle = findIliModelElement_Helper(roleDef, scopedNamePrefix, baseLanguage, scopedNameToFind);
					if (modelEle != null) {
						return modelEle;
					}
				}
			...
			while (contstraintI.hasNext()) {
				...
				ModelElement modelEle = findIliModelElement_Helper(constraint, scopedNamePrefix, baseLanguage, scopedNameToFind);
				if (modelEle != null) {
					return modelEle;
				}
			}
			...
			while (i.hasNext()) {
				...
				ModelElement modelEle = findIliModelElement_Helper(modelDef, scopedNamePrefix, baseLanguage, scopedNameToFind);
				if (modelEle != null) {
					return modelEle;
				}
			}
			...
			if (modelElement instanceof DomainDef) {
				...
				if (domain.containsType()) {
					...
					if (type instanceof Enumeration) {
						...
						ModelElement ele = findElementInEnumeration(scopedNamePrefix, baseLanguage, enumeration, scopedNameToFind);
						if (ele != null) {
							return ele;
						}
					}
	 ...
	 * @param scopedName Expected Scope Name
	...
	/**
	 * Check if an INTERLIS2Def is an internal one. An internal one
	 * is not exported.
	 * 
	 * @param obj

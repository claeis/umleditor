...
public class XMLInterlisEncoder
{
  ...
  private class GetterEntry {
    ...
    GetterEntry(String name,Method get,Method contains){this.name=name;this.get=get;this.contains=contains;}
  };
  ...
  private void analyzeClass(Class aclass)
  {
    ...
    if(getters.containsKey(aclass))return;
    if(iterators.containsKey(aclass))return;
    if(codelists.containsKey(aclass))return;
    if(aclass.getName().equals("java.lang.String"))return;
    if(isBuiltinClass(aclass))return;
    if(aclass.isPrimitive())return;
    ...
    for (int i = 0;i<methods.length;i++)
    {
      if(methods[i].getName().startsWith("get"))
      {
        ...
        }else if(methodSet.containsKey("getList") && methodSet.containsKey("createBuiltin") && methods[i].getName().endsWith("Code"))
        {
    ...
    throws IOException, IllegalAccessException, InvocationTargetException
  {
    ...
    if(isCodeList(obj.getClass()))return;
  ...
  throws IOException
  {
    ...
    /** prueft ob der Rueckgabewert einer Funktion eine Klasse der Art CodeList ist
     * @param method Methode die geprueft wird
     */
...
public class XMLInterlisEncoder {
	...
	// benÃ¶tigte Methoden werden hier in eine List gespeichert
	private void analyzeClass(Class aclass) {
		// class already analyzed?
	...
	private void visitObject(Object obj) throws IOException, IllegalAccessException, InvocationTargetException {
		...
		while (it.hasNext()) {
			...
			while (thisit.hasNext()) {
				value = (Object) thisit.next();
				// prÃ¼ft ob die Klasse vom Typ primitiv sei
				if (isBuiltinClass(value.getClass())) {
				} else if (value.getClass() == java.lang.String.class) {
				} else if (isCodeList(value.getClass())) {
				} else {
	...
	private void writeObject(Object obj) throws IOException, IllegalAccessException, InvocationTargetException {
		...
		while (it.hasNext()) {
			...
			while (thisit.hasNext()) {
				...
				// prÃ¼fen nach der Klasse vom Typ String
				else if (value.getClass() == java.lang.String.class) {
					out.write("<" + method.getName().substring(8) + ">" + value + "</" + method.getName().substring(8)
							+ ">");
					newline();
...
=======
	...
	/**
	 * containsXX() methode ausfÃ¼hren Hier wird geprÃ¼ft, ob die contains -
	 * Methoden TRUE sind, d.h. ob Objekte hinzugefÃ¼gt worde sind oder nicht
	 * 
	 * @param obj
	 *            Objekt auf dem die containsXX() Methode ausgefÃ¼hrt werden soll
	 * @param method
	 *            getXX()
	 */
	private boolean executeContains(Object obj, Method containsMethod)

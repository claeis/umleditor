...
public class TransferFromIli2cMetamodel {
	...
	private ch.ehi.interlis.graphicdescriptions.GraphicParameterDef findGraphicParameterDef(
			GraphicParameterDef gfxParam) {
	...
	private ch.ehi.uml1_4.foundation.datatypes.Multiplicity visitCardinality(Cardinality card) {
		...
		mr.setUpper(card.getMaximum() != Cardinality.UNBOUND ? card.getMaximum()
				: ch.ehi.uml1_4.implementation.UmlMultiplicityRange.UNBOUND);
	...
	private void visitAttribute(AttributeDef attrib, int attrIdx) {
		...
		if (btype instanceof CompositionType) {
			if (unwrapMultiValueStructAttrs && isMultiValueAttributeWrapper(btype)) {
				...
				if (iliAttrsAsUmlAttrs) {
					...
					destRole.setOrdering(type.isOrdered() ? ch.ehi.uml1_4.foundation.datatypes.OrderingKind.ORDERED
		...
		if (btype instanceof CompositionType) {
			if (unwrapMultiValueStructAttrs && isMultiValueAttr) {
				...
				LocalAttribute wrappedValue = (LocalAttribute) type.getComponentType().getElement(LocalAttribute.class,
						TransferFromUmlMetamodel.VALUE_ATTR);
				TypeAlias wrappedValueType = (TypeAlias) wrappedValue.getDomain(); // Type
																					// vom
																					// value
																					// Attribut
																					// innerhalb
			...
			if (attrib instanceof LocalAttribute) {
				...
				if (btype instanceof TypeAlias) {
					...
					if (predefinedBaseDomain == ilibase.BOOLEAN || predefinedBaseDomain == ilibase.HALIGNMENT
							|| predefinedBaseDomain == ilibase.VALIGNMENT || predefinedBaseDomain == ilibase.NAME
	...
	private void visitRoleDef(RoleDef role) {
		...
		roledef.setOrdering(role.isOrdered() ? ch.ehi.uml1_4.foundation.datatypes.OrderingKind.ORDERED
				: ch.ehi.uml1_4.foundation.datatypes.OrderingKind.UNORDERED);
		switch (role.getKind()) {
	...
	private void updateMappingToPredefinedModel(ch.ehi.uml1_4.modelmanagement.Package root) {
		...
		if (!ch.ehi.uml1_4.tools.NamespaceUtility.deepContainsOwnedElement(root,
				ch.ehi.interlis.modeltopicclass.ModelDef.class, ilibase.getName())) {
			// no need to link ili2c modelelemts to corresponding elements in
			// repository
			return;
		}

		// user may have moved ModelDef "INTERLIS" away from INTERLIS2Def
		// "predefined",
		// therefore do not try to find INTERLIS2Def "predefined"
		// ch.ehi.interlis.modeltopicclass.INTERLIS2Def
		// ili2Def=(ch.ehi.interlis.modeltopicclass.INTERLIS2Def)ch.ehi.uml1_4.tools.NamespaceUtility.deepGetOwnedElement(
		// root,ch.ehi.interlis.modeltopicclass.INTERLIS2Def.class,getPredefinedName());
		// fileMap.put(getPredefinedName(),ili2Def);

		ch.ehi.interlis.modeltopicclass.ModelDef modelDef = (ch.ehi.interlis.modeltopicclass.ModelDef) ch.ehi.uml1_4.tools.NamespaceUtility
				.deepGetOwnedElement(root, ch.ehi.interlis.modeltopicclass.ModelDef.class, ilibase.getName());
		...
		while (it.hasNext()) {
			...
			if (elt instanceof Unit) {
				ch.ehi.interlis.units.UnitDef unitDef = (ch.ehi.interlis.units.UnitDef) ch.ehi.uml1_4.tools.NamespaceUtility
						.deepGetOwnedElement(modelDef, ch.ehi.interlis.units.UnitDef.class, elt.getName());
				unitMap.put(elt, unitDef);
			} else if (elt instanceof Function) {
				ch.ehi.interlis.functions.FunctionDef funcDef = (ch.ehi.interlis.functions.FunctionDef) ch.ehi.uml1_4.tools.NamespaceUtility
						.deepGetOwnedElement(modelDef, ch.ehi.interlis.functions.FunctionDef.class, elt.getName());
				functionMap.put(elt, funcDef);
			} else if (elt instanceof Domain) {
				ch.ehi.interlis.domainsandconstants.DomainDef domainDef = (ch.ehi.interlis.domainsandconstants.DomainDef) ch.ehi.uml1_4.tools.NamespaceUtility
						.deepGetOwnedElement(modelDef, ch.ehi.interlis.domainsandconstants.DomainDef.class,
								elt.getName());
				domainMap.put(elt, domainDef);
			} else if (elt instanceof Table) {
				if (!((Table) elt).isImplicit()) {
					ch.ehi.interlis.modeltopicclass.ClassDef classDef = (ch.ehi.interlis.modeltopicclass.ClassDef) ch.ehi.uml1_4.tools.NamespaceUtility
							.deepGetOwnedElement(modelDef, ch.ehi.interlis.modeltopicclass.ClassDef.class,
									elt.getName());
	...
	private void visitMetaValues(ch.ehi.uml1_4.foundation.core.ModelElement ele,
			ch.ehi.basics.settings.Settings values) {
		if (values != null) {
			for (Iterator valuei = values.getValuesIterator(); valuei.hasNext();) {
				...
				umlTag.setName(new NlsString(TaggedValue.TAGGEDVALUE_LANG, TAGGEDVALUE_ILI_PREFIX + name));
	...
	private ch.ehi.interlis.domainsandconstants.DomainDef visitDomainDef(Domain dd) {
		...
		if (btype instanceof TypeAlias) {
			...
			if (predefinedBaseType == ilibase.BOOLEAN.getType() || predefinedBaseType == ilibase.HALIGNMENT.getType()
					|| predefinedBaseType == ilibase.VALIGNMENT.getType()
					|| predefinedBaseType == ilibase.NAME.getType() || predefinedBaseType == ilibase.URI.getType()) {
	...
	private ch.ehi.interlis.domainsandconstants.Type visitType(Container scope, Type dd) {
		...
		if (dd == ilibase.BOOLEAN.getType()) {
			...
			if (lt instanceof PolylineType) {
				...
				((ch.ehi.interlis.domainsandconstants.linetypes.IliPolyline) line)
						.setDirected(((PolylineType) lt).isDirected());
			...
			if (lineAttributeStructure != null) {
				((ch.ehi.interlis.domainsandconstants.linetypes.SurfaceType) line)
						.attachLinAttrDef(findClassDef(lineAttributeStructure));
			}
			...
			if (dd instanceof AnyOIDType) {
				...
				oid.attachOiddomain((ch.ehi.interlis.domainsandconstants.basetypes.BaseType) visitType(scope,
						((OIDType) dd).getOIDType()));
	...
	private ch.ehi.interlis.domainsandconstants.basetypes.NumericalType visitNumericalType(Container scope,
			NumericalType btype) {
	...
	private ch.ehi.interlis.domainsandconstants.basetypes.Enumeration visitEnumeration(
	...
	private ch.ehi.interlis.domainsandconstants.basetypes.EnumElement visitEnumerationElement(
	...
	private void visitElements(Container container) {
		...
		while (it.hasNext()) {
			...
			if (elt instanceof Model) {
				...
				/*
				 * Only explicit tables are printed out. Line attribute tables,
				 * for example, are not printed out.
				 */
				if (!((Table) elt).isImplicit()) {
			...
			if (nextUmlEle != null && lastUmlEle != null) {
				...
				ch.ehi.umleditor.application.ElementFactory
						.createDependency(ch.ehi.uml1_4.implementation.UmlUsage.class, nextUmlEle, lastUmlEle);
			}
			if (nextUmlEle != null) {
	...
	public void visitTransferDescription(ch.ehi.uml1_4.modelmanagement.Model umlModel, TransferDescription td,
			String configFilename, Configuration config) {
	...
	static java.util.ResourceBundle rsrc = ch.ehi.basics.i18n.ResourceBundle
			.getBundle(TransferFromIli2cMetamodel.class);

...
public class CreateDiagramUtility {
		...
		while (classi.hasNext()) {
			...
			if (!skipAssoc) {
				...
				while (rolei.hasNext()) {
					...
					java.util.Iterator pi = role.iteratorXorParticipant();
					while (!skipAssoc && pi.hasNext()) {
						ch.ehi.interlis.associations.Participant p = (ch.ehi.interlis.associations.Participant) pi
								.next();
						end = p.getParticipant();
	...
	static private ch.ehi.umleditor.umlpresentation.WayPoint createWayPoint(
			ch.ehi.umleditor.umlpresentation.Package p) {

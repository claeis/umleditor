...
public class HtmlWriter {
	...
	// fï¿½r die Nummerierung vom TopicDef, ModelDef und Interlis2Def
	int numeration = 0;

	// wird benï¿½tigt fï¿½r Stellen nach dem Komma: Package, Class, classDef
	int iddP;

	// used in pass STRUCTURE, changed in walkTreeRecursive()
	double level = 0.0;

	// Variable fï¿½r den Durchgang
	// man vergleicht nachher im doWerte ob pass==<eine der Konstanten>
	// und setzt die Reihenfolge, wie das HTML niedergeschrieben wird
	int pass;

	// ???Sringwert fï¿½r die die Leerschlï¿½ge, bzw. Abstï¿½nde in der Struktur
	String b = " ";
	...
	public void doObjectCatalog(Namespace apackage, java.io.Writer out)
			...
			// declare any checked exceptions
			// please fill in/modify the following section
			// -beg- preserve=yes 3CEE8A0A0302 throws3CEE891B03C7
			// "doObjectCatalog"
			throws java.io.IOException
	...
	// -end- 3CEE8B46037F throws3CEE891B03C7 "visitPackage"
	{
		// please fill in/modify the following section
		// -beg- preserve=yes 3CEE8B46037F body3CEE891B03C7 "visitPackage"
		// wird fï¿½r sectionNumber benï¿½tigt
		numeration = numeration + 1;

		// Wert es aktuellen Objekts
		String defLangName = encodeString(apackage.getDefLangName());

		// aName - wird fï¿½r die Angabe vom Link innerhalb der HTML - Datei
		// benï¿½tigt
		// bsp: <a name="aName....">
		String aName = numeration + "_" + defLangName;

		// ??concated value fï¿½r die Angabe der Werte+serialNumber, bzw. index
		// vom Value
		String value;
		if (suppressChNr) {
			value = defLangName;
		...
		// damit nur einmal die indexMap gefï¿½llt wird!
		if (pass == CONTENTS) {
			int sectionNumbers[] = new int[1];
			sectionNumbers[0] = numeration;
			indexMap.put((ModelElement) apackage, sectionNumbers);
		...
		if (linkElements) {
			...
			// concatedValue fï¿½r den Link innerhalb der HTML-Datei
			aNameStructure = Integer.toString(numerationId[0]) + "_" + defLangName;
		} else {
			concatedValue = defLangName;
		}
		...
		while (classi.hasNext()) {
			...
			if (!hasHeader) {
				...
				// iddP fï¿½r Package wird erhï¿½ht
				iddP++;
			}
		...
		while (classi.hasNext()) {
			...
			if (!hasHeader) {
				...
				// der Wert fï¿½r die Kommastellen wird hier erhï¿½ht
				iddP++;
			}
			String defLangClass = encodeString(((Classifier) obj).getDefLangName());
			if (pass == BODY) {
	...
	// -end- 3CEE8B57008A throws3CEE891B03C7 "visitClass"
	{
		...
		if (linkElements) {
			int numerationId[] = (int[]) indexMap.get((ModelElement) aclass);
			String numeration = Integer.toString(numerationId[0]) + "." + Integer.toString(numerationId[1]);
			// fï¿½r den Link innerhalb der HTML-Datei
				aName = numeration + "_" + classDefName;
			// concatedValue, dass geschrieben wird spï¿½ter
			if (suppressChNr) {
				value = classDefName;
			} else {
				value = numeration + " " + classDefName;
		...
		// Hier wird der Werte fï¿½r die Kommastellen auch erhï¿½ht
		// damit kommt man in eine neue Ebene, bzw. neuer Einzug
		iddP++;
	...
	/**
	 * Geht ï¿½ber die ganze Struktur. Die einzelnen ï¿½berschriften des HTML-Files
	...
	private void walkTree(Namespace apackage) throws java.io.IOException {
		...
		// Counters werden zu ihrem Ursprungswert zurï¿½ckgesetzt
		resetCounters();
	}
	...
	/**
	 * die INT - Werte, die fï¿½r die ID's der Element gebraucht werden

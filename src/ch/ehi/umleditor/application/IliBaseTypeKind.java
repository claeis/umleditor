...
public abstract class IliBaseTypeKind {
	...
	public static Vector getDomainDefTypes() {
		// kind=> alles gross: AttrType (COORD1..3, NUMERIC, TEXT, Enum,
		// HAlignment, VAlignment.Boolean, Basket ,Polyline ,Surface, Area
			...
			if (((AttributeDef) object).getAttrType() instanceof DomainAttribute) {
				...
				if (domainAttribute.containsDirect()) {
					...
					return domainAttribute.getDomainDef().getDefLangName()
							+ (tagDomainDef ? resIliBaseTypeKind.getString("CIDomainDefSuffix") : ""); //$NON-NLS-1$
				} else {

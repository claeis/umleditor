...
public class ExtendedPanel extends javax.swing.JPanel {
	...
	private boolean checkNameExtension(String name) {
		if (getChxExtended().isSelected()) {
			...
			// check if base class is in a topic that is the base of the topic
			// of this
			if (errorMessage == null) {
				...
				while (true) {
					...
					if (current == topicDefBase) {
						// ok; base class is in a topic that is the base of the
						// topic of this
						break;
					}
				}
			...
			if (errorMessage != null) {
				ch.softenvironment.view.BaseDialog.showError((java.awt.Component) this,
						resExtendedPanel.getString("CTInvalidExtension"), //$NON-NLS-1$
						errorMessage + "\n" + resExtendedPanel.getString("CWResetExtension"), null); //$NON-NLS-2$//$NON-NLS-1$
				getChxExtended().setSelected(false);
				return false;
			}
		}
	...
	/**
	 * connEtoC2: (CbxExtends.item.itemStateChanged(java.awt.event.ItemEvent)
	 * -->
	 * ExtendedPanel.cbxExtends_ItemStateChanged(Ljava.awt.event.ItemEvent;)V)
	 * 
	 * @param arg1
	...
	public boolean getExtension() {
		if (getCbxExtends().hasElementChanged()) {
			...
			if (parent != null) {
				...
				if (error != null) {
					BaseDialog.showWarning((java.awt.Component) this, resExtendedPanel.getString("CTNoGeneralization"), //$NON-NLS-1$
							error);
					return false;
				}
				try {

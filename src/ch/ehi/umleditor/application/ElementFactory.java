...
=======
	private static java.util.ResourceBundle resElementMapper = java.util.ResourceBundle
			.getBundle("ch/ehi/umleditor/application/resources/ElementFactory");
	 ...
	 * Create Composite without Edges.
	 */
	public static ch.ehi.umleditor.umlpresentation.Association createAssociationComposite(
			ch.ehi.uml1_4.foundation.core.Association associationDef, boolean showName) {
		// Create an Association Composite which connects 2 ClassFigure's as
		// follows:
		// ==> ClassFigure-Edge-Link-Edge-ClassFigure
		ch.ehi.umleditor.umlpresentation.Association associationComposite = (ch.ehi.umleditor.umlpresentation.Association) createObject(
				ch.ehi.umleditor.umlpresentation.Association.class);
		associationComposite.addSubject(associationDef);
	...
	/**
	 * Return a default AssociationDef link to two Roles Instances. Convention:
	 * The Association is added as OwnedElement to FromParticipant's parent.
	 * 
	 * @see #createAssociationDef(Element)
	 */
	public static AssociationDef createAssociationDef(GeneralizableElement participantFrom,
			GeneralizableElement participantTo) {
		// generic initialization
		AssociationDef associationDef = createAssociationDef(((ModelElement) participantFrom).getNamespace());
	...
	/**
	 * Return a default AttributeDef Instance. AttributeDef are added as Feature
	 * to AbstractClassDef.
	 * 
	 * @see #createObject(..)
	 */
	...
	/**
	 * Return a default UmlOperation Instance. Operations are added as Feature
	 * to AbstractClassDef.
	 * 
	 * @see #createObject(..)
	 */
	public static UmlOperation createUmlOperation(AbstractClassDef abstractClassDef) {
		...
		operation.setAbstract(false);
		operation.setLeaf(false); // successors may overwrite this method
		operation.setRoot(false); // predecessor may have implemented this
									// method
		operation.setOwnerScope(ScopeKind.INSTANCE);
		operation.setConcurrency(CallConcurrencyKind.SEQUENTIAL);
		operation.setVisibility(VisibilityKind.PUBLIC);
		...
		// define default datatype (TEXT*20)
		/*
		 * ch.ehi.interlis.attributes.DomainAttribute attrtype = new
		 * ch.ehi.interlis.attributes.DomainAttribute();
		 * ch.ehi.interlis.domainsandconstants.basetypes.Text text = new
		 * ch.ehi.interlis.domainsandconstants.basetypes.Text();
		 * text.setKind(ch.ehi.interlis.domainsandconstants.basetypes.TextKind.
		 * MAXLEN); text.setMaxLength(20); attrtype.attachDirect(text);
		 * attributeDef.attachAttrType(attrtype);
		 */
		// aggregations
		abstractClassDef.addFeature(operation);
	...
	/**
	 * Return a default UmlParameter Instance. Parameters are added as Parameter
	 * to Operations.
	 * 
	 * @see #createObject(..)
	 */
	...
	public static Contract createContract(ModelDef modelDef) {
		...
		// set defaults
		contract.setIssuer(new ch.ehi.basics.types.NlsString(resElementMapper.getString("CIContractIssuer"))); //$NON-NLS-1$
		// contract.setExplanation(new
		// ch.ehi.basics.types.NlsString("<Explanation>"));
	 ...
	 * @see #createObject(..)
	 */
	public static ch.ehi.uml1_4.foundation.core.Dependency createDependency(java.lang.Class dependencyClass,
			ModelElement client, ModelElement supplier) {
		// generic initialization
		ch.ehi.uml1_4.foundation.core.Dependency dependency = (ch.ehi.uml1_4.foundation.core.Dependency) createObject(
				dependencyClass);
	...
	public static ch.ehi.umleditor.umlpresentation.Diagram createDiagram(Element parent) {
		// generic initialization
		ch.ehi.umleditor.umlpresentation.Diagram diagram = (ch.ehi.umleditor.umlpresentation.Diagram) createObject(
				ch.ehi.umleditor.umlpresentation.Diagram.class);
		((Namespace) parent).addDiagram(diagram);
	 ...
	 * @see #createObject(..)
	 */
	public static ch.ehi.uml1_4.foundation.core.Generalization createGeneralization(GeneralizableElement parent,
	...
	public static ch.ehi.umleditor.umlpresentation.Note createNote() {
		...
		// (ModelElement)EditorTreeElement.createElement(Comment.class);

		// generic initialization
		ch.ehi.umleditor.umlpresentation.Note note = (ch.ehi.umleditor.umlpresentation.Note) createObject(
				ch.ehi.umleditor.umlpresentation.Note.class);
	...
	/**
	 * Create a generic Instance of given Class. Create default name if possible
	 * like "<ClassNameWithout ClassPath>".
	 * 
	 * @param className
	 */
	public static Object createObject(java.lang.Class className) {
	 ...
	 * Return a default PresentationEdge.
	 *
	 * The idea is to separate 1 UML-Association into 2 graphical edges which
	 * have an artifical link-node inbetween. This link-Node may be transformed
	 * for any Association-kind: - binary connection (2 nodes and 1 Association)
	 * - n-ary Association between > 2 nodes - extended binary Associations -
	 * Associations with Attributes
	 * 
	 * @see #createObject(..)
	 * @param classDiagram
	 *            diagram where new role is to be shown
	 * @associationComposite LinkNode's composite
	 * @node the new Classifier/-Figure to be connected
	 * @roleDef represented by node
	 */
	public static ch.ehi.umleditor.umlpresentation.PresentationRole createPresentationRole(
			ClassDiagramView classDiagram, ch.ehi.umleditor.umlpresentation.Association associationComposite,
			ch.ehi.umleditor.umlpresentation.PresentationNode node,
			ModelElement /* AssociationEnd || Participant */ roleDef) {
		ch.ehi.umleditor.umlpresentation.PresentationRole edge = (ch.ehi.umleditor.umlpresentation.PresentationRole) createObject(
				ch.ehi.umleditor.umlpresentation.PresentationRole.class);
		edge.addEndpoint(associationComposite.getLinkPresentation()); // first
		edge.addEndpoint(node); // second
		edge.addSubject(roleDef);
	...
	/**
	 * Create an XOR-Constraint between to AssociationEnd's. If an existing role
	 * of an association gets an XOR-Constraint to another Class (ClassDef) a
	 * Participant instance maps the XOR-Role-Constraint.
	 *
	 * Definition by the UML/INTERLIS-Editor: - concerns the very same
	 * Association => no alternate Association for XOR-edge
	 * 
	 * @param existingRoleDef
	 * @param targetClass
	 */
	...
	public static void removeElement(Element element) {
		/*
		 * if (element instanceof ch.ehi.uml1_4.foundation.core.Association) {
		 * ch.ehi.uml1_4.foundation.core.Association association =
		 * (ch.ehi.uml1_4.foundation.core.Association)element;
		 * 
		 * // remove the parent Namespace parent = association.getNamespace();
		 * parent.removeOwnedElement(association);
		 * 
		 * // remove both AssociationEnd's incl. their Participant's
		 * java.util.Iterator iterator = association.iteratorConnection(); //
		 * from AssociationEnd associationEnd1 =
		 * (AssociationEnd)iterator.next(); associationEnd1.detachParticipant();
		 * // to AssociationEnd associationEnd2 =
		 * (AssociationEnd)iterator.next(); associationEnd2.detachParticipant();
		 * // remove roles association.removeConnection(associationEnd1);
		 * association.removeConnection(associationEnd2); } else
		 */
		if ((element instanceof ModelElement) && ((ModelElement) element).containsNamespace()) {
			Namespace parent = ((ModelElement) element).getNamespace();
			// parent.removeOwnedElement((ModelElement)element);
			...
			// remove from iteratorClientDependency
			ModelElement client = (ModelElement) ((ch.ehi.uml1_4.foundation.core.Dependency) element).iteratorClient()
					.next();
			((ch.ehi.uml1_4.foundation.core.Dependency) element).clearSupplier();
			client.removeClientDependency((ch.ehi.uml1_4.foundation.core.Dependency) element);
		} else if (element instanceof ch.ehi.uml1_4.foundation.core.Generalization) {

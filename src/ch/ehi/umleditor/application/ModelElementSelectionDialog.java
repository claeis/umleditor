...
=======
	 ...
	 */
	 private void addEscapeKey() {
		 ...
		 Action escapeAction = new AbstractAction() {
			...
			private static final long serialVersionUID = -1581209471420018044L;
	 ...
	 *            boolean
	 */
	public ModelElementSelectionDialog(java.awt.Dialog owner, String title, boolean modal,
	 ...
	 *
	 */
	public ModelElementSelectionDialog(java.awt.Dialog owner, String title, boolean modal, ModelElement context,
	...
	public ModelElementSelectionDialog(java.awt.Dialog owner, String dlgTitle, String lstTitle, boolean modal,
			ModelElement context, Set referenceableElements) {
		super(owner, dlgTitle, modal);
		initialize();
		getLblClassName().setText(lstTitle);
		fillList(context, referenceableElements);
...
=======
	 ...
	 * --> ModelElementSelectionDialog.cancelPressed()V)
	 * 
	 * @param arg1
	 *            java.awt.event.ActionEvent
	 */
...
=======
	...
	/**
	 * connEtoC3:
	 * (LstModelElements.listSelection.valueChanged(javax.swing.event.ListSelectionEvent)
	 * --> ModelElementSelectionDialog.lstModelElements_ValueChanged()V)
	 * 
	 * @param arg1
	...
	/**
	 * Fill the list with possible references. The List will be sorted according
	 * to ModelElement name with package-name successor, such as: aTopic
	 * (ch.ehi.interlis.modeltopicclass) bTopic (ch.softenvironment.topics) :
	 */
	private void fillList(ModelElement modelElement, java.util.Set set) {
		referencables = new java.util.TreeSet(new ModelElementComparator());
		referencables.addAll(set);

		// create displayable list (which contains a null Element more in
	...
	private void fillList(ModelElement modelElement, java.lang.Class referenceClass) {
		...
		java.util.Set set = new HashSet();
		java.util.Set candidates = ch.ehi.interlis.tools.ModelElementUtility.getReferencableElements(modelElement,
				referenceClass);
		java.util.Iterator iterator = candidates.iterator();
		while (iterator.hasNext()) {
			Object referencableObject = iterator.next();
			// select box for IMPORTS or DEPENDS ON? // TODO: refactor; caller
			// should clearer signal what kind of list he asks for
			if (referenceClass == TopicDef.class || referenceClass == ModelDef.class) {
				// sort out already referenced elements
				java.util.Iterator iteratorDependency = null;
				if (referenceClass == ModelDef.class) {
				...
				while (!found && iteratorDependency.hasNext()) {
					...
					while (!found && iteratorSupplier.hasNext()) {
						Object supplier = iteratorSupplier.next();
						if (supplier.equals(referencableObject)) {
							// referencableObject is a referenced element
							// already
							// do not offer it in select box
							found = true;
						}

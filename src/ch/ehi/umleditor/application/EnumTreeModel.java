...
/**
 * Implements an adapter to the metamodel as required by the
 * IliBaseTypeEnumPanel's JTree.
 *
 * @author ce
 */
public class EnumTreeModel implements TreeModel, ch.ehi.uml1_4.changepropagation.MetaModelListener {
	...
	/**
	 * adapts MetaModelChanges to TreeModelEvents. see also getChildren() for a
	 * similar structure
	 */
	...
	private static boolean isAttribute(String ops, String attr) {
		...
		if (ops.equals("attach" + attr))
			return true;
		if (ops.equals("detach" + attr))
			return true;
		if (ops.equals("set" + attr))
			return true;
		if (ops.equals("add" + attr))
			return true;
		if (ops.equals("remove" + attr))
			return true;
		if (ops.equals("set" + attr))
			return true;
		if (ops.equals("clear" + attr))
			return true;
		if (ops.equals("change" + attr))
			return true;
		if (ops.equals("_link" + attr))
			return true;
		if (ops.equals("_unlink" + attr))
			return true;
		return false;
	}
